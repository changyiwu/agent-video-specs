...
- **教訓**：`dur` 取**兩個下限的較大者**——`旁白 + 2~4s` 與 `動畫完成 + 1.5s`（規範 5.2「動畫結束後靜止 1.5s 以上」）。
- **範本已機制化，不必自己記**：`PAGES` 每頁多一個 `anim` 欄位，由**寫動畫的人宣告**該頁分階動畫跑完的秒數（最後一階延遲 + 該階 transition，例如 `setTimeout(fn, step*2500)` 五階配 `transition: opacity .6s` 就是 `4×2.5 + 0.6 = 10.6`）。兩道關卡自動擋：`generate_narration.py` 的稽核表會列出來並 `exit 1`（master.mp3 不產出）、`record.cjs` 在開錄前中止（不必白錄整支才發現）。沒有分階 reveal 的頁不用寫。
- **為什麼是宣告而不是自動偵測**：靜態分析猜不準——宣告在非 `.slide-N` 選擇器上的 `transition` 抓不到，任意控制流的 `setTimeout` 也剖析不了。半正確的檢查比沒有檢查危險，因為它會讓人以為驗過了。所以**改動畫節奏時要同步改 `anim`**，這是這套機制唯一要人守的紀律。
...
- **教訓**：**Playwright 與所有 node_modules 一律裝在非 GDrive 路徑**（如 `%TEMP%/avs-render/`），再把範本複製回 GDrive；或直接用 `npx`（不下載依賴）。

### D-2 🔧 Playwright 錄製需 `NODE_PATH` 指向非 GDrive 的 node_modules
- **現象**：`node record.cjs` 報 `MODULE_NOT_FOUND`（GDrive 內刻意沒裝 playwright）。
- **原因**：Playwright 裝在 `%TEMP%/avs-render/` 而非專案內。
- **教訓**：執行前設 `$env:NODE_PATH = "$env:TEMP\avs-render\node_modules"`，或直接在 temp 目錄跑腳本。
...
- [ ] 改過動畫節奏的話，`PAGES` 的 `anim` 有同步改？（C-7）
